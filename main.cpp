...
uintptr_t GetModBase(DWORD procId, const char* modName)
{
	...
	{
		do {
			if (!_stricmp(entry.szModule, modName))
			{
				modBaseAdr = reinterpret_cast<uintptr_t>(entry.modBaseAddr);
				break;
			}
...
uintptr_t baseAddress = GetModBase(GetProcId("csgo.exe"), "client.dll");
DWORD PID = GetProcId("csgo.exe");
HANDLE hprocess = OpenProcess(PROCESS_ALL_ACCESS, FALSE, GetProcId("csgo.exe"));

template <typename T> T memRead(SIZE_T address) {
	T buffer;
...
void draw(HDC hdc)
{
	...
	uintptr_t localPlayer = memRead<uintptr_t>(baseAddress + offsets::dwLocalPlayer);


	view_matrix_t vm = memRead<view_matrix_t>(baseAddress + offsets::dwViewMatrix);
	int localteam = memRead<int>(memRead<DWORD>(baseAddress + offsets::dwLocalPlayer) + offsets::m_iTeamNum);
	...
	for (int i = 1; i < 64; i++)
	{
		...
		uintptr_t pEnt = memRead<DWORD>(baseAddress + offsets::entityList + (i * 0x10));



		int health =memRead<int>(pEnt + offsets::m_iHealth);
		int team = memRead<int>(pEnt + offsets::m_iTeamNum);
		...
		if (memRead<int>(localPlayer + offsets::m_iHealth) < 0 || memRead<int>(localPlayer + offsets::m_iHealth) > 100) {
			continue;
		}

		Vector3 pos = memRead<Vector3>(pEnt + offsets::m_vecOrigin);
		Vector3 head;
		head.x = pos.x;
		head.y = pos.y;
		head.z = pos.z + 75.f;
